...
export const LogsPrivateSaleMessage = async (props: Props): Promise<MessageCreateOptions> => {
    return {
        content: `||${props.user}||`,
        embeds: [
            new Discord.EmbedBuilder()
                ...
                .addFields(
                    ...
                    {
                        name: `**${emojis.accept} | Produtos entregues:**`,
                        value: `\`\`\`${props.checkout.stocks?.map(s => s.content).join(' \n')}\`\`\``
                    },
                    {
